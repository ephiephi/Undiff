...
# def train_noisemodel(root, network="NetworkNoise2",epochs=1800, dataset_size=1, n_samples=4,batch_size=2,g_t=None, num_nets=200,min_epochs=400,slope_epochs=2,test_len=1,mog=0,lr=None,one_network=False):
...
#             logging.info("starting training")
#             nets,loss_array,loss_test_array = train_nets_parralel(network,train_full_tensors, test_full_tensors,trial,epochs=epochs,num_nets=num_nets,batch_size=batch_size,g_t=g_t,exp_root=exp_root,min_epochs=min_epochs,slope_epochs=slope_epochs,mog=mog,lr=lr,one_network=one_network)
...
# def organize_wav_files_snr_levels(exp_root, output_pickle, snr_levels, num_train_seconds=0, num_test_seconds=10, num_train_seconds_noise=0, num_test_seconds_noise=10):
#     """
#     Organize files into directories based on file IDs (X), calculate and scale noise for multiple SNR levels,
#     save the results, and save metadata to a DataFrame.
#     """
...
#     parser.add_argument(
#         "-config",
#         default="exps_configs/o_net6dmog1.yaml",
#     )
...
#     # else:
#     #     names, noises_names,snr_array = organize_wav_files_snr_levels(exp_root, output_pickle_path, snr_array, num_train_seconds=test_start_sec, num_test_seconds=test_end_sec,  num_train_seconds_noise=test_start_sec_noise, num_test_seconds_noise=test_end_sec_noise)
    
#     run_network=None 
#     # if trained_model_path == "0":
#     #     logging.info("---startin training---")
#     #     train_noisemodel(exp_root, network=network,epochs=epochs, dataset_size=dataset_size, n_samples=n_samples,batch_size=batch_size,g_t=g_t,num_nets=num_steps,min_epochs=min_epochs,slope_epochs=slope_epochs,test_len=test_len,mog=mog,lr=lr,one_network=one_network)
#     # else:
#     #     run_network=  network
#     names=["6","18"]
#     noises_names = ["6","18"]
