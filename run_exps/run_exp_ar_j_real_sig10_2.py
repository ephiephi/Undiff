...
mainroot = Path("/data/ephraim/datasets/known_noise/undiff/exp_ar_j_real_2sec_sig10")
roots = [ mainroot/"b"] #,,mainroot/"b",mainroot/"c"
for root in roots:
    print("root: ", root)
    ...
    for wav in tqdm(noiswavs):
        ...
        for j in [0]:
            ...
            # s_array1 =[0.01,0.08,0.1, 0.12,0.15]
            s_array1 = []
            s_array2=[0.2,0.3,0.5,0.11,0.8]
            ...
            for s in s_array: 
                ...
                if not os.path.exists(newOUTPATH):
                    os.mkdir(newOUTPATH)
                command = "HYDRA_FULL_ERROR=2 CUDA_VISIBLE_DEVICES=2 python main.py model=diffwave task=unconditional output_dir=results/guided audio_dir=1 guid_s={} y_noisy={} outpath={} s_schedule={} noise_type={} noise_model_path={}".format(s,wav, newOUTPATH, s_schedule, "loss_model_sig10",noise_model_path) #CUDA_VISIBLE_DEVICES=0
                print(command)
                os.system(command)
