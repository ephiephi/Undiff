...
def train_nets_parralel(network,train_dataset, test_dataset,trial=0, epochs=100,num_nets=200,batch_size=16):
    results = []
    gpu_num=4
    ...
    with mp.get_context('spawn').Pool(processes=4) as pool:
        args = [(network, train_dataset, test_dataset,devices[i % gpu_num], idxes, trial,epochs,batch_size) for i, idxes in enumerate(idxes_all)]
...
def train_noisemodel(root, network="NetworkNoise2",epochs=60, dataset_size=128*8, n_samples=640000,batch_size=16):
    print("starting ----")
    ...
    for i in snr_df2.index:
        ...
        for trial in [0]:
            ...
            nets,loss_array,loss_test_array = train_nets_parralel(network,train_full_tensors, test_full_tensors,trial,epochs=epochs,num_nets=200,batch_size=batch_size)
...
if __name__ == '__main__':
    ...
    with open("exps_configs/m_ar_short.yaml", "r") as f:
    ...
    train_noisemodel(exp_root, network=network,epochs=epochs, dataset_size=dataset_size, n_samples=n_samples,batch_size=batch_size)#128*8

    print("---run_exp---")
    # names=["j","b","c"]
    run_exp(exp_root, dirnames=names, cuda_idx="2",s_array=s_array)
