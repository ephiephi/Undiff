...
for root in roots:
    ...
    for wav in tqdm(noiswavs):
        ...
        for j in [0]:
            ...
            s_array2=[4.5,5,5.5,6,6.5,7,7.5,8,8,10,12]#[0.18,0.19,0.2,0.21,0.22,0.23,0.24,0.25,0.3,0.35,0.4,0.5,0.7,0.8,1,1.2,1.5,2,3,4]
            # s_array3 = list(np.logspace(-2,0.7,num=40))
            s_array= s_array1 + s_array2#+s_array3
            # s_array = [1]###############
            ...
            for s in s_array: 
                ...
                if not os.path.exists(newOUTPATH):
                    os.mkdir(newOUTPATH)
                command = "HYDRA_FULL_ERROR=1 CUDA_VISIBLE_DEVICES=1 python main.py model=diffwave task=unconditional output_dir=results/guided audio_dir=1 guid_s={} y_noisy={} outpath={} s_schedule={} noise_type={} noise_model_path={}".format(s,wav, newOUTPATH, s_schedule, "loss_model",noise_model_path)
                print(command)
                os.system(command)
