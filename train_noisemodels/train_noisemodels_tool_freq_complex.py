...
def train_nets(train_dataset):
    ...
    for i,model in enumerate(models):
        ...
        stds_re = torch.std(stft.real,dim=1)
        stds_im = torch.std(stft.imag,dim=1)
        means_re = torch.mean(stft.real,dim=1)
        means_im = torch.mean(stft.imag,dim=1)

        stats = (stds_re, stds_im,means_re,means_im)
...
def create_dataset_real(noise_whole, sr):
    # noise_sample1 = noise_whole[:, int(sr/2):sr]
    # noise_sample2 = noise_whole[:, 0:int(sr/2)]
    noise_sample1 = noise_whole[:, 0:int(5*sr)]
    noise_sample2 = noise_whole[:,  int(5*sr):int(10*sr)]
    print("shape(noise_sample2): ",(noise_sample2.shape))
...
if __name__ == "__main__":
    print("starting ----")
    root = "/data/ephraim/datasets/known_noise/undiff/exp_ar_j_real_freq_complex/"
